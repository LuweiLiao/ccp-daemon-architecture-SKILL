...
description: "CCP Daemon (Continuous Closed Probe) — 24/7 自治流水线架构 v4。三用户隔离（hermes/ccpd/ccrunner）+ CC done marker 协议 + ManagedProcess 跨 tick 恢复 + /proc/pid/stat starttime 校验 + diff 白名单检查 + SQLite 状态机。"
---

# CCP Daemon — 24/7 自治循环流水线 v4
...
| **v4 (当前)** | **三用户权限模型 + CC done marker 协议 + ManagedProcess 跨 tick 恢复 + /proc/pid/stat starttime 校验 + diff 白名单检查** |
...
class CCPDaemon:
    ...
    def _do_verify(self, attempt_id):
        ...
        if state["is_alive"]:
            ...
            self._on_verify_fail(attempt_id)
